...
5. **純前端四合一多格式匯出**：匯出 UTF-8 `.md`、內嵌完整樣式與向量 SVG 的單一自給離線 `.html`、高解析列印級防截斷 `.pdf`、以及 **3x Retina 超高解析度 PNG 圖片長圖匯出**（支援手機直式 412px 適配 6.3 吋 18:9、平板直式 834px 適配 11 吋 4:3 與自適應寬度，自動同步主題背景並精確至秒時間戳記命名）。
